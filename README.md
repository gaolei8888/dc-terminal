...
- 会话停掉后 worktree 目录不会自动删除，留在 `<repo>/.git/dct-worktrees/` 下，需要手工 `git worktree remove` 清理。这是有意的——会话停掉往往正是出问题的时候，那一刻不该把现场清掉。
- agent 会话绑定一个目录直到结束，中途不能换目录。换目录就是开新会话。
