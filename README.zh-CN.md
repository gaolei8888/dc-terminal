...
**DC 没有申领页面**：它的密钥由疯狂AI训练营发给上课的人。它排在第一位就是为了这件事——
对第一天上课的学生来说，这一整张表里只有这一行不需要注册、不需要银行卡、不需要翻墙。
不上课的人当它不存在就行，或者把它指到自己的地址上：它就是一个普通 profile，
在 `~/.dct/profiles/` 里放一份自己的 `dc.toml` 就覆盖掉了。
