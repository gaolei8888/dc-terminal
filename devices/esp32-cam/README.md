...
- **振幅 3000（满格的约 9%）是默认音量，也是上限**：3500、4000、8000 都破音（用 440Hz 测）；2000 不破音但用户嫌小。
  **用板上麦克风量过（喇叭放、麦克风同时录，看基频响度和 2、3 次倍频占比）**，同样振幅 3000：

  | 频率 | 440 | 660 | **880** | 1000 | 1320 | 1500 | 2000 | 2500 |
  |---|---|---|---|---|---|---|---|---|
  | 基频响度 | 38,802 | 163,066 | **331,032** | 280,965 | 78,624 | 85,435 | 85,709 | 115,772 |
  | 倍频占比 | 27.9% | 2.8% | **1.1%** | 1.7% | 11.4% | 6.1% | 10.4% | 16.1% |

  **这个喇叭的甜区是 660–1000Hz**：880Hz 比 440Hz 响 8.5 倍、几乎不破。440Hz 又小又破，用户听到的「频率不对 / 破音」就是它。
  主频每一档都是对的（没有翻倍或减半）。耳朵觉得「开始破」的点，正好在倍频占比 30–35% 之间。
  **放旋律也照这个选音域**：《欢乐颂》移到 D 调（主要音 587–880Hz）比《小星星》放在 523–880Hz 明显更响更干净（用户实听确认）。
  ⚠️ 880Hz 下把振幅从 3000 加到 8000，录到的响度反而下降、倍频不升——麦克风离喇叭太近，被震饱和了，这一段量不准，要靠耳朵。
