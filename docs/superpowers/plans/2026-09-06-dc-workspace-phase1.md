...
`workspace/` 里**必须**有一份单人版 compose：`git clone` 之后 `docker compose up`
...
- [ ] **验收（这一期最重要的一条）**：拿一个没装过任何东西的环境，只给一条链接，
      走完「打开 → 看板 → 配对 → 开出一个能干活的 agent」。中间任何一步要求他
      在本地装东西或改配置，这一期就没达标
- [ ] 浏览器打开能看到完整看板
...
- [ ] **本仓库到此为止。** 「三十个学生怎么发、反代、证书、配额」是部署方的事
      （见上面那节）。本仓库这边只再交付一样：`workspace/README.md` 里那份
