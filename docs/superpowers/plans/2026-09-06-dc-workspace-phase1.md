...
新目录 `workspace/`，留在**本仓库**里，不另开一个 repo。理由跟 `dct-page` 那段
「整个仓库里只有这一份」是同一条：Dockerfile 装的是这个仓库编出来的二进制，
分开放两个 repo，迟早出现镜像里那份和代码里那份对不上、而且只有在容器里才复现的 bug。

```
workspace/
  Dockerfile          # 多阶段：构建 dct → 运行时镜像
  entrypoint.sh       # 起守护进程、起 ttyd、把 SIGTERM 传下去
  compose.yaml        # 一人一容器的样板，含卷和内存 limit
  gate/               # 任务 3 那道门（第二期原样复用）
  README.md           # 两种代码位置的代价表，给带训练营的人看
```
