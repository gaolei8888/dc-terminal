...
### 任务 4.5：容器里粘不了图，而它现在的说法像是用户的错 ✅（2026-09-07）

**浏览器那一层补不上，查过了，所以这一期做的是「把话说准」那条最低验收线。**
要让图片从浏览器进来，得让 ttyd 的前端页面接住 `paste` 事件再传上来，而那一页
是 ttyd 二进制里内嵌的**单文件 730 KB**，换它等于把它整份 vendor 进仓库、跟着
版本走；那道门（`gate.rs`）又是一根**刻意不解析 HTTP 正文**的字节管道，在它
身上改写页面会把「不解析所以没有走私问题」那条保证一起拆掉。两条都不划算。

- [x] 至少做到不撒谎。原来的返回类型是 `Option<PathBuf>`，`None` 同时背着
      两个意思：「剪贴板里不是图」和「这个平台根本读不了剪贴板」，而界面对
      两者说同一句「剪贴板里没有图片」。拆成三态 `Pasted{Image,NoImage,
      Unsupported}`，让调用方**没法**再用同一句话糊过去
- [x] 新词条 `ImagePasteUnsupportedHere`：「这个环境里粘不了图——把图片存成
      文件，再把路径打进去」。给的退路是真的：`paste_image` 送给 agent 的
      本来就是一条路径
- [x] 那 4 条 dead-code 警告没了。**不是靠删代码**：那三个辅助函数在
      mac/windows 和测试里都用得上，加的是一个说明白理由的 `cfg_attr`。
      在容器里重编了一次确认——Linux 上零警告
- [x] 新测试 `a_platform_without_image_paste_says_so_instead_of_blaming_the_clipboard`
      只在非 mac/windows 上编，所以**在这台 Windows 机器上永远不会跑**——
      特地进容器里跑了一遍 Linux 版，4 条全过

**顺带发现：这个仓库没有跑测试的 CI。** `.github/workflows/` 里只有
`release.yml`（打 tag 出三平台产物），没有任何一个 job 跑 `cargo test`。
所以上面那条 Linux-only 的测试目前只有人手动进容器才跑得到。这不在本期范围
里，但计划里任务 5 那句「要在本仓库的 CI 里跑」是建立在一个不存在的东西上的。
...
### 任务 6：一人一容器（本仓库这一半）✅（2026-09-07）

- [x] **单人版 compose 原来起不了第二份。** 端口写死 7681，第二份直接
      `Bind for 127.0.0.1:7681 failed: port is already allocated`——而计划里
      写着「发三十个学生就是跑三十次」，那句话在写死端口的版本上**是假的**。
      真跑了一次才发现。改成 `${DCW_HOST_PORT:-7681}`，`DCW_PUBLIC_URL` 跟着
      它走，所以第二份印出来的链接也是对的
- [x] **起一份就产出一条能直接发出去的链接**（任务 3 那道门印在日志里）
- [x] `mem_limit` 按实测定，不拍脑袋：底座 22.6 MiB，加一个闲着的 claude
      会话 105 MiB（净增约 82）。但闲着的 agent 不能拿来定 limit——干活时是
      README 里那个约 320 MB/agent，按「一人同时开三个」算 ≈ 1 GB，2 GiB 是
      一倍余量
- [x] **验收**：起两份，互相看不见对方的会话和密钥

隔离到什么程度，一条条量过：

| | |
|---|---|
| 钥匙 | 各一把，互相打不开对方的门（都是 401） |
| 会话 | `dct ps` 只看得见自己的 |
| 密钥仓 / 对话记录 / 代码 | 三个卷各一套，`secrets.toml` 的 md5 不同 |
| 文件系统 | 各自只看得到自己的 `/home/dc` |
| **网络** | **通的** |

**最后一行是这一条最重要的发现，而且跟我原先的假设相反。** 从学生 2 的容器里
能连上学生 1 的 `7681`——compose 给两个项目建了两个网络，但它们之间是通的。
拦住他的**是那道门不是网络**：不带钥匙 401，带自己的钥匙也是 401。而真正能
给出终端的 ttyd 在 `7682` 上只绑容器内环回，邻居连不到——**所以「ttyd 绑
127.0.0.1」那个决定是承重的，不是好看**。

这条也划清了一条边界：**本仓库不提供网络隔离，README 里明写了别假设它有。**
同一台机器上跑很多份且互相不信任的话，`internal` 网络或防火墙规则是起容器
的人的事。

- [x] **本仓库到此为止。** 契约清单在 `container/README.md`
