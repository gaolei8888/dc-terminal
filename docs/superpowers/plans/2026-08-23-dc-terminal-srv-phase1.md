...
### 任务 3：daemon 出网（`src/link.rs`）✅（2026-08-28）

- [x] 独立线程，`ureq` 长轮询 `POST /link/poll`，空转即重发
- [x] 收到信封 → 解出 `Request` → **走调用方传进来的那个 dispatch**（就是
      `daemon.rs` 的 `handle`，`src/web` 那条路用的也是它）→ 答复包成信封 POST 回去
- [x] 断线：指数退避重连
- [x] **没有单独的心跳**，见下面第二条
- [x] 线程 panic 不许拖垮守护进程（`catch_unwind`，同 `bridge.rs::spawn`）
- [x] 测试：8 条，含一个假 srv（`127.0.0.1:0`）跑通 `List` 往返、头五条请求全失败后自己回来
- [x] 验收：网络调用全在 `link.rs` 自己的线程上，tick 一行都没碰

**四件计划没说清楚、做的时候必须定的事：**

1. **`POLL_TIMEOUT` 挪进了 `dct-link`。** 中转拿它决定挂多久，守护进程拿它算
   自己的读超时，而**读超时必须更长**——短了的话每一次轮询都在中转开口之前被
   客户端自己掐掉，症状是「怎么都连不上」而两边日志都干净。所以读超时是算出来的
   （`POLL_TIMEOUT + 10s`）不是填出来的，有测试盯着方向。

2. **不做心跳。** 计划写「心跳 45 秒」，但长轮询 30 秒就返回一次、守护进程立刻
   再发一条，这条连接上每 30 秒必有一次往返，本来就短于 45 秒。再加一个心跳
   定时器是两套机制干同一件事，而两套机制会各自超时、各自重连。

3. **`Envelope.seq` 的注释原来是自相矛盾的**，顺手改了（**没动线上形状**）：
   写的是「请求和答复配对用；同一个 `from` 上单调递增」——可答复的 `from` 是
   另一头，它身上的 seq 就不递增了。真正的规则是：**由发起方生成、在发起方
   身上递增，答复原样带回同一个 seq**。答复带回 seq 有测试盯着。

4. **本任务不改 `daemon.rs`。** 「File Structure」把「起 link 线程」记在
   `daemon.rs` 名下，但那要有中转地址和令牌才起得来，而那两样是任务 6（配对）
   的产物。现在接上去只能接一个永远配置不出来的死开关。`link.rs` 自己是完整的，
   `spawn(Link::new(cfg, dispatch))` 一行就能接，**任务 6 接**。

变异测试：seq 不带回、答复回给自己、退避没有上限、连上了不归零、读超时短于长轮询、
不去掉地址结尾的斜杠、解析失败也回成功、退避时不看叫停——八个全挂。最后那个第一轮
**没挂**：原来那条测试把退避上限设成 40ms，`run` 循环顶上那次检查就够了，根本碰不到
`nap` 里的判断。改成退避 5 秒、只给 2 秒等待之后才真的测到它。
