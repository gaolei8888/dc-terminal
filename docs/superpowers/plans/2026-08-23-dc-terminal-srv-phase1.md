...
### 任务 4：手机网页 —— 换一个数据来源 ✅（2026-08-29，登录除外）

拆成了两半做，理由见下。**登录那一条按你的决定留给任务 5。**

**4a：把"数据从哪儿来"收成一处**

- [x] 页面里六件事（文案、列表、画面、打字、按键、翻历史）收进一个 `wire`，八处调用全改
- [x] 局域网行为一个字没变，第 0 期那些页面守卫原样继续绿

**4b：中转那条路**

- [x] 页面挪进 `crates/dct-page`，`dct` 和 `dct-srv` 引用同一个常量；有测试逐字节比对两边发的字节
- [x] `/link/ask`：发一个信封、挂着等同 `seq` 的答复，一次往返
- [x] `relayWire`：请求装进信封走中转，**返回形状跟局域网那份完全一样**
- [x] 断线两种说法：`Offline`（没连上）和 `NoAnswer`（连着但不回话）
- [ ] 登录：dc_classroom 账号 → 设备列表 → 选一台（**任务 5**）

**跟计划写的不一样的四处：**

1. **页面不是"挪进 srv"，是挪进两边共用的 `crates/dct-page`。** 照原文挪走会把第 0 期
   的局域网模式直接删掉——那是已经在用的功能；拷一份则更糟，两份网页各自演化，其中
   一份的 bug 只在另一种模式下复现，而那时候没人会想到去对比两个文件。

2. **改动远不止"取数的那几行"。** 页面在两种模式下要发的是同一批 `Request`，可路由层
   当时还留着三处"顺手算一下"，而中转手上只有信封、一处也做不了：取 i18n 文案表、
   把 `zh-CN` 认成 `Lang::Zh`、把键名翻成字节。三处都还成了协议（`Request::WebStrings`、
   `Request::Key`，协议 9 和 10）。这不是新设计，是 `routes.rs` 自己开头那句
   「这一层不许有业务逻辑……这也是第 1 期能只换一根管子的前提」在还债。

3. **手机不用长轮询收答复，用 `/link/ask` 一次往返。** 另一条路是手机也挂长轮询、
   自己按 `seq` 配对；那份簿记会落在这个仓库唯一跑不了测试的地方（页面里一行 JS 都
   执行不了，全靠 Rust 扫源码的守卫钉着）。放在中转里它有测试，也做得了变异测试。

4. **选哪台电脑暂时看 `#d=`。** 第一期的临时办法，任务 5 接上登录之后就该整个消失，
   页面注释里写着别在它上面盖东西。

**变异测试**：页面那边四个（键名当文本发、只用局域网那份、文案请求名写错、列表请求名
写错），中转那边五个（按寄件人配对、seq 配错一位、超时不清挂号、投不出去不清挂号、
没回话谎报离线）。**其中两个第一轮活了下来**——「键名当文本发」和「永远只用局域网那份」，
两处都是"只在另一种模式下坏"的 bug，补了守卫才抓住。经中转那份发什么请求，现在是从
**真类型**序列化出来对的，不是手打清单：`Request::Key` 在 Rust 这边改名，测试立刻红。
