...
日期：2026-08-04
状态：设计已确认；先决条件 0.2（`ui.rs` 拆分）已随九宫格那轮做完并合入 main，
0.1（协议握手版本号 + `dct restart`）和滚屏本身都还没开始
...
房规是「一句没说清下一步该干嘛的错误提示，不算写完」。所以提示里那个
`<命令>` 必须真的存在，得同时加一个 `dct restart` 子命令。

它不能走协议——协议不匹配正是它要解决的问题。做法是按可执行文件的绝对路径
杀掉旧守护进程（`pkill -f "^<current_exe> daemon$"`），再拉一个新的。老守护进程
不需要认识任何新东西，这条路就能走通。

**重启会杀掉所有正在跑的会话，agent 干到一半的活会中断。** 所以 `dct restart`
必须先把会话列出来再问一句。`Request::List` 在新老两版里是同一个变体，
对着旧守护进程也能问出来——这一步不需要握手成功。

```
重启会关掉这 2 个正在跑的会话：
  #4  claude   ~/work/dc/dc-terminal   干活中
  #5  codex    ~/work/dc/dc_desktop    空闲
它们干到一半的活会中断。确定要重启吗？[y/N]
```

### 0.2 `ui.rs` 拆分（已完成）

> 已随九宫格那轮做完并合入 main。落地的模块比下面列的多一个
> `src/ui/app.rs`（第二步的 `App` 结构自己占一个文件），另外
> `src/ui/grid.rs` 是九宫格带来的，不属于本次拆分。测试数量那条约束按
> 「拆分前后不变」执行，当时的绝对值是 191 而不是文中写的 172——文档写下时
> 的数字早被后来的活儿超过了，约束本身没变。
...
- `src/client.rs`：握手版本相等时正常返回；版本不等时错误文案里同时出现两个
  版本号和 `dct restart`；守护进程回 `Error("请求解析失败: …")` 时走同一条提示
  （模拟老守护进程——起一个只会回这一句的假 socket，不需要真守护进程）
- `dct restart`：会话清单的成句是纯函数，单独测；用户回 `n` 时不杀任何东西
