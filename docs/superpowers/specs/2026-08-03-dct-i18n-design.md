...
dct 的界面文案全是硬编码的中文——生产代码里 **154 条**用户可见的中文字面量
（不含注释与测试），其中 96 条在 `src/ui/` 下。非中文用户开起来是一片看不懂的方块。

> 2026-08-04 复核：条数与本文写就时基本没变。`src/ui.rs` 已拆成 `src/ui/{mod,view,board,
> grid,pick,attach,secret,widgets,app}.rs`，本文里出现的 `src/ui.rs:NNN` 一律按此换算；
> `src/session.rs` 的行号也已漂移，文案本身仍在（`没有这个 profile:` 现在是 119 行，
> `目录不存在:` 137，`不是 git 仓库` 150，`没有这个会话:` 220，`还没有检查点` 349，
> `checkpoint_base` 的两句在 326 / 333）。
...
> **2026-08-04 更新：这一节已经被实现取代，`fit_help` 不要再做了。**
>
> D（看板按项目过滤）那一期给底栏加了 `a` 键，撞上同一个问题并已修掉：底栏从 3 行
> 改成 4 行（两行文字），折行交给 `widgets::wrap_help`——只在分隔符处断，**一条都不丢**。
>
> 它比本节原来设计的 `fit_help` 更好：`fit_help` 是按优先级**丢掉**放不下的键，
> 而被丢掉的键在屏幕上不留痕迹，用户没有任何线索知道自己少看了几个。德语那种长译文
> 只会让它丢得更多。`wrap_help` 则是把所有键都折到第二行，代价只是内容区少一行。
>
> 所以 i18n 这一期在排版上**不需要做任何事**：译文变长时 `wrap_help` 自然多折一行，
> 而「翻译的人不必为长度操心」这个目标已经达成。真正要做的只剩下面那条对齐的旧账。

顺带修掉一个既有毛病：几处列宽用 `{:<N}` 按**字符数**补齐，而 `truncate` 按**显示宽度**
截断。中文 4 个字符、显示宽 8，右边那列就被推歪。现存四处：`board.rs:88`（`{:<8}` 状态词，
「干活中」正是中文）、`board.rs:91`（`{:<10}` profile 名，目前都是 ASCII，但译文可能不是）、
`board.rs:95`（`{:<22}` 路径）、`pick.rs:455`（`{:<20}` 项目名，中文目录名会踩）。
全部改成 `widgets::pad_to`——它已经是按显示宽度补齐的，跟 `truncate` 同一份宽度定义。
