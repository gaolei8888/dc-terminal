...
实现期间发现底部状态栏有两个既有缺陷，与本设计改的是同一段代码，一并修（见计划 Task 4）：

**（a）底部提示不分视图。** 进了会话视图，底部仍显示看板的按键表
（`n 新建  ↑↓ 选择  Enter 进入  u 回滚 …`），而这些键在会话视图里**全部转发给 agent**。
用户照着按 `n`，字母 n 落进 agent 的输入框。显示做不到的操作比不显示更糟。
改成提示跟着视图走。

**（b）逆转键与标题栏不一致，且偷了 `Esc`。** 实测：`src/ui.rs:232` 截走的是 `Esc`，
而 `src/ui.rs:417` 的标题栏写着「Ctrl+B 返回看板」——按 Ctrl+B 什么也不会发生。
`ff1e37d` 改了文案和测试注释，没改按键处理。后果是 Claude Code 里按 `Esc` 取消不掉任何东西。

**裁定：逆转键改成 `F2`，`Esc` 与 `Ctrl+B` 一律还给 agent。** Esc 是 agent 的取消键，
`Ctrl+B` 是 Claude Code 的「转后台」。F2 没有 CLI agent 在用，也就不需要
tmux 那种「双击透传」的隐形状态——对非程序员更直白。
