...
读循环用 DA1 哨兵（`\x1b[c`）保证 OSC 11 回复不会漏到事件循环里被当成按键，但这
是唯一一层防线，还有两条路没堵上：

- 读循环有 256 字节上限，可能在 DA1 回复到达前就把 buf 交出去（比如启动时用户在
  疯狂敲键，把这段配额提前占满）。
- 有的终端/多路复用器自己在本地应答 DA1、却把 OSC 11 转发给上游终端处理——这样
  「终端按顺序应答」的前提就不成立，DA1 到了不代表 OSC 11 回复已经落定。

另外，`StdinReader::read_reply` 本身没有会真正执行它的测试：探测逻辑的单测全部
喂的是 `CannedReader` 这个测试替身，`libc::poll`/`libc::read` 从来没跑过——I/O 那
一半只验证了「读出来的东西会被正确解析」，没验证「读」本身。
