...
/// 一次长轮询挂多久。**两边必须知道同一个数**，所以它在这里而不在任何一边：
///
/// - 中转拿它决定挂多久之后空手打发人走；
/// - 守护进程拿它算自己的读超时，**那个必须更长**。反过来的话，每一次轮询都
///   会在中转开口之前被客户端自己掐掉，看起来就是「怎么连都连不上」，而两边
///   的日志都显示自己没错。
///
/// 30 秒是照着运营商 NAT 的回收时间挑的（spec「断线」那一节）：挂得比它久，
/// 连接会被中间某个盒子悄悄掐掉，而两端都不知道。也正因为轮询本身每 30 秒
/// 就是一次往返，**这条链路不需要另外的心跳**——计划里写的「心跳 45 秒」由
/// 这个数字一并满足了，再加一个定时器就是两套机制在干同一件事。
...
pub struct Envelope {
    ...
    /// 请求和答复配对用。
    ///
    /// **由发起方生成**，在发起方自己身上单调递增；答复原样把同一个 `seq`
    /// 带回来。（原来这里写的是「同一个 `from` 上单调递增」——那跟"答复要
    /// 带回同一个 seq"是矛盾的：答复的 `from` 是另一头，它身上的 seq 就不
    /// 递增了。配对是这个字段的用途，递增只是发起方那一侧的实现方式。）
    pub seq: u64,
