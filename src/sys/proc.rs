...
//!
//! 那些 agent 进程会不会因此变成孤儿，原本只能推：ConPTY 的伪控制台句柄
//! 随进程一起被内核关掉时，挂在上面的客户端进程应当跟着结束。这一条**至今
//! 仍然没有在真机上验过**，而它现在也不必验了——兜底已经在。每个会话的子
//! 进程都套了一个带 `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` 的 job object
//! （`sys::job`）：守护进程被 `TerminateProcess` 时，内核替它关掉那个句柄，
//! 圈里的进程一起走。所以换守护进程漏不漏 agent，不再取决于那条推断。
//!
//! 促成 `sys::job` 的**不是**上面这条推断，是另一件真在机器上撞见的事：
//! 2026-09-05 实测 `dct kill` 打印了「已杀」，而那个会话的 `claude.exe`
//! 还活着——Windows 上我们 spawn 到的是 `cmd.exe /c ...\claude.CMD`，
//! `TerminateProcess` 只认句柄不认血缘，打到的是外面那层壳。原来这里写着
//! 「别在验之前先写」，那条没有被绕过：动手的理由是验过的那件事，换守护
//! 进程这条路只是顺带被同一个机制盖住了。
use std::io;
