...
pub(crate) enum PairPhase {
    ...
    Waiting {
        ...
        /// **浏览器到底弹出来没有。**
        ///
        /// 上面 `url` 那段注释里已经写着「浏览器打不开是常态」，可标题那句
        /// 长期是无条件的「在刚打开的页面里输入这个码」——于是打不开的时候，
        /// 屏幕最上面那句话说的是一件没发生过的事，纠正只在底栏一行红字里。
        /// 用户是从上往下读的。
        ///
        /// 远程版（浏览器里的看板）上这不是边缘情况而是**必然**：容器里没有
        /// 浏览器可开，每一个学生都会撞上。所以这件事必须进阶段本身，让
        /// `phase_line` 分两句说，而不是让一句话去赌哪种情况更常见。
        browser_opened: bool,
    },
...
///
/// 目标用户是非程序员：让他每次在十个 agent 里挑一个是设计失败——他不知道区别。
...
pub(crate) fn idle_help(view: &View, lang: Lang, ctx: HelpCtx) -> Vec<HelpItem> {
    ...
    match view {
        ...
        // 里有九项按下去只会被拒，`g` 才是这一屏唯一走得通的路，它不能是
