...
pub(crate) fn handle_key(app: &mut App, key: KeyEvent) -> Result<()> {
    ...
    match key.code {
        ...
        // 在跑的会话」的组（守卫在 `unpin_current` 里），那种组在九宫格里
        // 一个格子都没有，少了这个键用户只能先 `g` 回列表才拿得掉。
        //
        ...
        //
        // **被拒绝时一动不动。** `unpin_current` 拒绝还有在跑的会话的组，
        // 底栏写着「这个项目还有会话，先停掉才能移除」——那句话说的是「什么
        // 都没发生」，而这时候焦点可能是陈旧的（`Tab` 到一个空项目上、焦点
        // 故意留在别人家的格子里，随后后台那一轮 `List` 把一个活会话捎进了
        // 这个项目）。照样对齐光标的话，当前项目会被那个陈旧焦点悄悄换走，
        // 用户接着按 `n`，会话开进了另一个项目，而屏幕刚刚告诉过他这一下
        // 没生效。
