...
///
/// 焦点可以是明知陈旧的。九宫格里有些落点根本没有格子可指，那时候焦点只能
/// 留在原处（见 `grid::focus_first_of_current_group`）：`Tab`/数字键走到一个
/// 没有活会话的项目、`p` 刚摆上一个新项目、`home_view` 因为光标停在组头或
/// 已停止会话上而回落到第 0 格。这几种情况里 `focus` 指的都是**上一个**项目，
/// 拿它去改写光标，就等于用户刚换过去的项目被 `g` 一声不响地换了回来，
/// 而底栏上一秒还写着新项目名。
///
/// **判据问的是「焦点是不是陈旧的」，不是「这个组有没有活格子」。** 这两个
/// 问题只在一种场景下答案相同，往两个方向都会岔开，而且两边都咬人：
///
