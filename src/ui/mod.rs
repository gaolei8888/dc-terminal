...
///
/// **光标所在的项目在九宫格里一个格子都没有时，这里什么都不做。**
/// 那种项目是真实可达的（`p` 摆上来还没按 `n`，或者会话全停了），`Tab` 走到
/// 它上面时焦点**故意**留在原处——没有格子可指，硬挪就会指到别人家的格子上
/// （见 `grid::focus_first_of_current_group`）。于是那一刻的 `focus` 是明知
/// 陈旧的，它指着上一个项目；再拿它去改写光标，就等于用户刚按 `Tab` 换过去
/// 的项目被 `g` 一声不响地换了回来，而底栏上一秒还写着新项目名。
///
/// 判断从**数据**上现算（当前组有没有活会话），不另立一个「焦点是陈旧的」
/// 标志位：标志位得在每一次焦点移动、每一次 `refresh_rows` 收拢焦点之后
/// 记得清掉，漏一处就又是同一个 bug，而且不会有任何编译期信号。现算的这条
/// 在**所有**调用点上同时成立，包括以后新加的出口——新出口不必知道有这条
/// 规矩存在，它也已经被守住了。
///
/// 代价说清楚：停在这样一个项目上时，方向键只挪焦点、不改「当前项目」，
/// 底栏会一直写着那个空项目而 `▶` 在别人家的格子里。这是刻意的——用户是
/// 专门 `Tab` 过去准备在那儿开会话的，浏览一眼别人的画面不该把 `n` 的落点
/// 悄悄挪走。再按一次 `Tab`／数字键，或者那个项目一有会话，就都回正了。
pub(crate) fn sync_board_cursor_from_grid(app: &mut App) {
    if app
        .current_group()
        .is_some_and(|g| g.first_live().is_none())
    {
        return;
    }
    if let View::Grid { focus, .. } = app.view {
